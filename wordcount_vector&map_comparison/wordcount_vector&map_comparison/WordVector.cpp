...
void WordVector::insert(const string text)
{
	...
	{
		while (it != data.end()) //loop through the vector
		{
			if (it->get_text() < text)  //increment the iterator if the text of Word to be inserted is greater than the text of Word in the vector
			{
				it++;
			}
			else if (it->get_text() > text) //insert the Word if the text of Word to be inserted is less than the text of Word in the vector
			{
				data.insert(it, w);
				break;
			}
			else                     //increment the count of the Word if it already exists
			{
				it->increment_count();
				break;
			}
		}
		if (it == data.end())        //insert at the end of the vector if it is greater than all the Words in the vector
		{
			data.push_back(w);
		}
	}
